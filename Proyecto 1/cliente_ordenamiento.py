...
def casi_ordenado(n):
    ...
    while k <= (n//4):
        ...
        arrg[numeroAleatorio],arrg[numeroAleatorio+4] = arrg[numeroAleatorio+4],arrg[numeroAleatorio]
        k += 1
...
def cargar_algoritmos_de_modulos():
    ...
    de ordenamiento que se encuentran en el modulo <<ordenamiento>>, y en la lista (funtions_list)
    
    Retorna:
        Una lista con las funciones del modulo ordenamiento y la lista (funtions list)
    """
    actual_functions = ['mergesort','heap_sort','quicksort_randomizado','Quicksort_median_of_3','introsort','quicksort_three_way_partition','dual_pivot_quicksort']
    functions_list = [o for o in inspect.getmembers(ordenamiento)
                      if inspect.isfunction(o[1]) and o[0] in actual_functions]
...
def resultados_algoritmos(n_elems, algs, n_pruebas, t_prueba):
    """
